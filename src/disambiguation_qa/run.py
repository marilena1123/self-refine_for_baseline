...
def iterative_disambiguation_qa(input_text: str, max_attempts: int, engine: str) -> dict:
    task_init = DisambiguationQATaskInit(
        engine=engine, prompt_examples="data/prompt/disambiguation_qa/init.jsonl"
    )
    task_feedback = DisambiguationQAFeedback(
        engine=engine, prompt_examples="data/prompt/disambiguation_qa/feedback.jsonl"
    )
    task_iterate = DisambiguationQATaskIterate(
        engine=engine, prompt_examples="data/prompt/disambiguation_qa/feedback.jsonl"
    )
...
def run_over_data(
    ...
    for _, row in data.iterrows():
        try:
            logs = iterative_disambiguation_qa(
...
def parse_args():
    parser = argparse.ArgumentParser(description="Run self-refine on disambiguation QA")
    parser.add_argument(
        "--model",
    ...
    parser.add_argument(
        "--input_file",
        default="data/tasks/disambiguation_qa/disambiguation_qa.jsonl",
        help="Input JSONL file with 'input' and 'target' fields",
    )
    parser.add_argument(
        "--output_file",
        default="data/tasks/disambiguation_qa/output.jsonl",
