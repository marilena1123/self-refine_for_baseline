...
from src.bbh_snarks.task_init import BBHSnarksInit
from src.bbh_snarks.feedback import BBHSnarksFeedback
...
def iterative_snarks(question, target, max_attempts, temperature):
    task_init = BBHSnarksInit(
        engine=ENGINE,
        prompt_examples="data/prompt/bbh_snarks/init.txt",
        temperature=temperature,
    )
    task_feedback = BBHSnarksFeedback(
        engine=ENGINE,
...
def run(data_file, max_attempts, num_samples, outfile, temperature):
    ...
    for i, ex in enumerate(tqdm(examples, desc="Snarks")):
        question = ex["input"]
        target = ex["target"]

        print(f"\n[{i}] Q: {question[:80]}... Target: {target}")
        try:
            log = iterative_snarks(question, target, max_attempts, temperature)
            if log is None:
...
if __name__ == "__main__":
    ...
    parser.add_argument("--data_file", type=str, default="data/tasks/bbh_snarks/snarks.json")
    parser.add_argument("--max_attempts", type=int, default=4)
    parser.add_argument("--num_samples", type=int, default=0)
    parser.add_argument("--outfile", type=str, default="data/outputs/bbh_snarks_results.jsonl")
